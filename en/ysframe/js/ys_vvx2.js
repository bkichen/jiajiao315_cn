    ...
    $('.defaultForm').bootstrapValidator({
        ...
        fields: {
            ...
            password: {
                validators: {
                    notEmpty: {
                        message: 'Please enter a 6-16 digit password'
                    },
                    stringLength: {
                        min: 6,
                        max: 16,
		...
		$("#myButton").button('reset');
				...
				$.ajax({
							type : "post",
							url : "/uc/a/student/busiStudent/checkUser?jsoncallback=?",// 获取产品信息列表
							dataType : "jsonp",
							data : {
								password : password,
								username : username
							...
							success : function(data) {
								if (data.flag == 1) {
									...
									alert("This account has been frozen, banned logging！");
									$("#myButton").dequeue();
									$("#myButton").button('reset');
								} else {
...
$(document).ready(function() {
    $('.defaultForm1').bootstrapValidator({
    	...
    	$.ajax({
			type:"post",
			url:"/uc/a/student/busiStudent/checkEmailAndTele?jsoncallback=?",//获取产品信息列表
			dataType:"jsonp",
			data:{
				email:email,
				telephone:telephone,
			...
			success : function(data) {
				if(data.flag == 1){
					...
					alert("This phoneNumber has been registered");
				}else if(data.flag==3){
					alert("Registration is successful, the page is about to jump!");
